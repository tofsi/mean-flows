...
model.load_state_dict(torch.load('qvae/enc_model/model.epoch-2'))
...
for epoch in range(epochs):
    ...
    for chunk_idx, chunk_file in enumerate(chunk_files):
        ...
        for batch in pbar:
            try:
                loss, kl, wacc, tacc, sacc, _ = model(batch, beta)  # beta > 0 for VAE
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        if (chunk_idx + 1) % 5 == 0:
            ...
            }, f"qvae/var_model/checkpoint_e{epoch}_c{chunk_idx}.pt")
