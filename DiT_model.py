...
class LabelEmbed(nn.Module):
    ...
    def __call__(self, labels, train: bool = True):
        ...
        embed = nn.Embed(num_embeddings=self.num_classes + 1, 
...
class PositionalEmbed(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        pos_embed = get_2d_sincos_pos_embed(hidden_dim, int(num_patches**0.5))  # (N, D)
...
class DiTBlock(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        attn_out = nn.SelfAttention(num_heads=self.num_heads,
                                qkv_features=self.hidden_dim,
                                out_features=self.hidden_dim,
                                use_bias=True)(attn_in)
...
class FinalLayer(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        x = nn.Dense(features=self.patch_size * self.patch_size * self.out_channels,
...
class DiT(nn.Module):
   ...
   # LabelEmbed, DiT blocks, adaLN-Zero and FinalLayer
    ...
    '''
    # Patch embedding
    x_emb = PatchEmbed(self.patch_size, self.in_channels, self.hidden_dim)(x) # (B, N, hidden_dim)
    # Positional embedding
    x_emb = PositionalEmbed(x_emb.shape[1], self.hidden_dim)(x_emb) # (B, N, hidden_dim)
    # Time embeddings
    t_emb = TimeEmbed(self.hidden_dim)(t)  # (B, hidden_dim)
    r_emb = TimeEmbed(self.hidden_dim)(r)  # (B, hidden_dim)
    # Label embeddings
    y_emb = LabelEmbed(self.hidden_dim, self.num_classes, drop_prob=0.1)(y, train=train)  # (B, hidden_dim)
...
class TimeEmbed(nn.Module):
    ...
    def __call__(self, t):
        ...
        freqs = jnp.exp(
            -jnp.arange(half_dim) * (jnp.log(10000.0) / half_dim)
        )
...
class MLP_with_GELU(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        x = nn.Dense(features=int(self.hidden_dim*self.mlp_ratio),
...
class AdaLNZero(nn.Module):
    ...
    def __call__(self, h):
        ...
        h = nn.Dense(self.hidden_dim*parts, 
                    kernel_init=nn.initializers.zeros,
