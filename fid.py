# Stolen with permission from https://github.com/kvfrans/jax-fid-parallel/blob/main/fid.py
...
def _preprocess_to_inception(x):
    """
    x: (B, H, W, 3) in [0,1] or [-1,1]
    returns (B, 299, 299, 3) in [-1,1], float32
    """
    x = jnp.asarray(x, jnp.float32)
    # assume [0,1] from your VAE decode; if already [-1,1], skip this
    if x.max() > 1.5:
        x = x / 255.0
    # [0,1] -> [-1,1]
    x = x * 2.0 - 1.0
    # resize to 299x299
    x = jimage.resize(x, (x.shape[0], 299, 299, 3), method="bilinear")
    return x


def make_fid_feature_extractor():
    """
    Returns a function: images (B,H,W,3) -> (B, 2048) features.
    Works on single GPU; under the hood uses pmap but hides sharding.
    """
    apply_pmap = get_fid_network()  # this is pmap'd over local_devices
    n_devices = len(jax.local_devices())

    def extract_feats(images):
        """
        images: np.array or jnp.array, shape (B,H,W,3)
        returns np.array (B, 2048)
        """
        x = _preprocess_to_inception(images)  # (B,299,299,3)
        B = x.shape[0]
        # shard over devices: (n_devices, B_per_dev, 299,299,3)
        if B % n_devices != 0:
            # pad to multiple of n_devices
            pad = n_devices - (B % n_devices)
        ...
        x = x.reshape(n_devices, B_pad // n_devices, 299, 299, 3)
        feats = apply_pmap(x)  # (n_devices, B_per_dev, 1, 1, 2048)
        feats = feats.reshape(B_pad, -1)  # (B_pad, 2048)
